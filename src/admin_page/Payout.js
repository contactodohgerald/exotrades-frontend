...
function Payout() {
    ...
    const getListOfWithdrawals = (pageNumber = 1) => {
        axios.get(BASE_URL+`get-unconfirm-withdrawals?page=${pageNumber}`, {
            headers: {
    ...
    const declineUserPayment = async (values, {resetForm}) => {
        await axios.post(BASE_URL+'decline-withdrawal', values, {
             headers: {
                 'Authorization': `Bearer ${API_TOKEN}` 
             }
         })
    ...
    return withdrawals === null ? (
        ...
        <Stack>
                    ...
                    <Tbody>
                    {
                        ...
                        withdrawals.data.map((each_withdraw, indexs) => (
                            <Tr key={indexs}>
                                ...
                                <Td>
                                    <Button variant={'outline'} colorScheme={'grey'} onClick={() => bringoutDeclineModal(`${each_withdraw.unique_id }`)}>Decline</Button>
                                </Td>
                                <Td>
                                    <Moment format="DD/MM/YYYY">{each_withdraw.created_at}</Moment>
                                </Td>
                                <Td>
                                    <Link to={'/payout-processor/'+each_withdraw.unique_id }>
                                        <Button variant={'solid'} colorScheme={'teal'}>Payout</Button>
                                    </Link>
                                </Td>
                            </Tr>
                        )) :
            ...
            <Modal isOpen={isOpen} onClose={onClose}>
                ...
                <ModalOverlay>
                    <ModalContent>
                        <ModalHeader fontSize='lg' fontWeight='bold'>Decline Payout</ModalHeader>
                        <ModalCloseButton/>
                        <ModalBody>
                        <Box>
                            <Text>Are you sure you want to decline this user's payout?</Text>
                            <Formik initialValues={initialValue} validationSchema={validationSchema} onSubmit={declineUserPayment}>
                            {
                                formik => {
                                    return <Form>
