...
function VerifyNewAccount() {
    ...
    const onSubmit = (values, {setErrors}) => {
        axios.post(BASE_URL+`verify-verification-token/account-activation/${params.user_id}`, values)
        .then(respond => {
    ...
    const resendTokenHandle = () => {
        axios.get(BASE_URL+`send-verification-token/${params.user_id}`)
        .then(respond => {
            //return the data from the api on success
            const res = respond.data
            if(res.status === false){
    ...
    return (
        ...
        >
            ...
            p='8px 20px' 
            rounded='md'>
                <Heading as='h3' size='lg'>Verify Account</Heading>
                <Divider mt="5" orientation='horizontal' />
                <Alert status='success' variant='subtle'>
                    <Text textAlign={'center'}>Thanks for signing up! Before getting started, head over to your email and provide the token that was sent to you? If you didn't receive the email, we will gladly send you another</Text>
                </Alert>
