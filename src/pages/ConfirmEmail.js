...
function ConfirmEmail() {
  ...
  const onSubmit = (values, {setErrors}) => {
    axios.post(BASE_URL+'send-reset-password-token', values)
    .then(respond => {
      ...
      navigate('/verify-token/'+data.unique_id, {replace: true})
    })
    .catch(error => {
    ...
    <Container>
      ...
      >         
        ...
        <Box 
          ...
          p='8px 20px' 
          rounded='md'>
            <Heading as='h3' size='lg'>Confirm Email</Heading>
            <Divider mt="5" orientation='horizontal' />
            <Alert status='success' variant='subtle'>
                <Text textAlign={'center'}>A token will be sent to the provided email for reseting your password</Text>
            </Alert>
            <Divider mt="5" orientation='horizontal' />
            <Formik initialValues={initialValue} validationSchema={validationSchema} onSubmit={onSubmit}>
              {
                formik => {
                  return <Form>
                      ...
                      <FormikControl control='chakrabutton' type='submit' label='Verify Email' disabled={!formik.isValid || formik.isSubmitting}/>
                  </Form>
                }
              }
            </Formik>
